...
export function LoadingBar(props: LoadingBarProps): JSX.Element {
  ...
  const [leftTwo, setLeftTwo] = useState({
    position: -100,
    transition: "left 0.5s linear",
  });

  const calculateNewValue = (
    currVal: AnimationStatus,
    setNextVal: Dispatch<SetStateAction<AnimationStatus>>,
  ) => {
    const position = currVal.position >= 100 ? -100 : currVal.position + 25;
    const transition =
      currVal.position >= 100 ? "left 0s linear" : "left 0.5s linear";
